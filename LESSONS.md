...
| **v5.10** | Khắc phục lỗi ép chữ dọc từng ký tự (Ảnh 1) & Tách bạch 'Hôm nay' với việc cũ trong Database (Ảnh 2) | 1. **Chống nén chữ dọc**: Tái cấu trúc card trong Columns/Board: đưa toolbar thao tác lên floating hover góc trên và tách hàng badge xuống dưới, giải phóng 100% bề rộng cho tiêu đề nhiệm vụ không bao giờ bị ép thành 1 chữ cái/dòng.<br>2. **Chuẩn hóa cột Today Database**: Phân biệt rạch ròi danh mục `daily` và ngày thực tế `todayKey`; bổ sung công tắc chuyển đổi `Hôm nay (Today)` vs `Tất cả việc ngày (All Daily)` ngay trên đầu cột; gắn badge ngày tháng rõ ràng (`📅 Hôm nay`, `📅 DD/MM Trễ`); tích hợp thanh thông báo việc tồn đọng kèm nút bấm 1 chạm chuyển việc sang hôm nay. |
